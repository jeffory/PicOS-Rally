...
void sim_step(car_t *car, const sim_input_t *in, const tuning_t *tun, int surface) {
    ...
    float mu = sim_surface_mu(surface);
    float mu_r = in->handbrake ? mu * 0.55f : mu;
    float ca_f = tun->ca_front;
    float ca_r = in->handbrake ? tun->ca_rear * 0.4f : tun->ca_rear;
